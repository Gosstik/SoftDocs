...
`boot mode` &mdash; алгоритм, определяющий тип загрузки операционной системы в память.
Разновидности `boot mode`:

- `CMS` (Legacy BIOS, Compatibility Support Module) &mdash; использовался раньше
- `UEFI` (EFI, ESP, Unified Extensible Firmware Interface) &mdash на современных компьютерах, замена `CMS`

`UEFI BIOS` поддерживает два режима загрузки: режим загрузки `Legacy BIOS` и режим загрузки `UEFI`. Обратная совместимость нужна, потому что некоторые устройства и операционные системы пока не поддерживают новый `UEFI` и могут загружаться только в режиме загрузки `Legacy BIOS`.
